...
<<<<<<< HEAD
  ...
  executeSlash: async (interaction) => {
    try {
      ...
      const timeString = interaction.options.getString('duree');

      // Parser la durée (format: 1j, 12h, 30m)
      const timeMatch = timeString.match(/^(\d+)(j|h|m)$/i);
      if (!timeMatch) {
        return interaction.editReply({ content: '❌ Format de temps invalide.\nUtilisez: `1j` (jours), `1h` (heures), ou `1m` (minutes)\nExemple: `12h`' });
      }
      ...
      if (timeValue <= 0) {
        return interaction.editReply({ content: '❌ La durée doit être un nombre positif.' });
      }
      ...
      const embed = new EmbedBuilder()
        ...
        .setFooter({ text: `Organisé par ${interaction.user.username}`, iconURL: interaction.user.displayAvatarURL() })
        .setTimestamp(endTime)
      ...
      const updateInterval = setInterval(async () => {
        ...
        await updateGiveawayEmbed(latest, interaction.client);
      }, 60000);

      scheduleGiveawayEnd(giveaway, interaction.client, updateInterval);
