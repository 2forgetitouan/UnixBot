...
app.post('/move-command', (req, res) => {
  ...
  try {
    ...
    log('MOVE', `Commande "${command}" déplacée de "${from}" vers "${to}"`, { user: req.ip });
    ...
    log('ERROR', `Erreur move-command: ${error.message}`, { user: req.ip });
...
async function notifyAuthAttempt({ req, user, status, reason }) {
  ...
  try {
    ...
    await target.send(`Tentative de connexion AxonBot\n${details}`);
  } catch (err) {
...
app.delete('/giveaway/:messageId', async (req, res) => {
  try {
    ...
    log('DELETE', `Giveaway "${giveaway.prize}" annulé`, { user: req.ip });
...
app.put('/giveaway/:messageId', async (req, res) => {
  ...
  giveawayModule.loadGiveaways();
    ...
    log('UPDATE', `Giveaway "${updated.prize}" mis à jour`, { user: req.ip });
...
app.post('/bot-start', (req, res) => {
  ...
  bot.startBot()
    .then(() => {
      log('SUCCESS', 'Bot démarré depuis l\'interface web', { user: req.ip });
      res.json({ success: true, message: 'Bot démarré avec succès' });
    })
    .catch(err => {
      log('ERROR', `Erreur démarrage bot: ${err.message}`, { user: req.ip });
...
app.post('/bot-stop', (req, res) => {
  ...
  bot.stopBot()
    .then(() => {
      log('SUCCESS', 'Bot arrêté depuis l\'interface web', { user: req.ip });
      res.json({ success: true, message: 'Bot arrêté avec succès' });
    })
    .catch(err => {
      log('ERROR', `Erreur arrêt bot: ${err.message}`, { user: req.ip });
      res.status(500).json({ success: false, message: err.message });
    });
});
...
app.post('/bot-restart', (req, res) => {
  ...
  bot.restartBot()
    .then(() => {
      log('SUCCESS', 'Bot redémarré depuis l\'interface web', { user: req.ip });
      res.json({ success: true, message: 'Bot redémarré avec succès' });
    })
    .catch(err => {
      log('ERROR', `Erreur redémarrage bot: ${err.message}`, { user: req.ip });
      res.status(500).json({ success: false, message: err.message });
    });
});
